...
In the :overlay mode, which is the default, local variables are still usable. 
...
For Ruby 1.8.7 or higher, :ctx_only mode is available. This means only the context is available to unsafe code. In case you want to publish local variables to the safe block, you have to pass them as parameters.
...
JRuby does not support $SAFE levels, so we're pretty much out of luck.
